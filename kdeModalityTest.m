...
%       close all
  ...
  if ~isempty(E)
    if isempty(modalityFits)
      ...
      pValDiff = pval;
      pValFit = 1 - pval;
      resultsTable = [numel(E); h(iH); S_k; stat; pval; 1 - pval];
    ...
    elseif (resultsTable(1,end) == numel(E) && resultsTable(6,end) < pval) ||...
      ...
      pValDiff(end) = pval;
      pValFit(end) = 1 - pval;
      resultsTable(:,end) = [numel(E); h(iH); S_k; stat; pval; 1 - pval];
    ...
    elseif resultsTable(1,end) < numel(E)
      ...
      pValDiff = [pValDiff pval];
      pValFit = [pValFit 1 - pval];
      resultsTable = [resultsTable [numel(E); h(iH); S_k; stat; pval; 1 - pval]];
